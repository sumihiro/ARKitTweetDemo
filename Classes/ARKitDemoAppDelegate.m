...
- (void)applicationDidFinishLaunching:(UIApplication *)application {    
	...
	ARGeoTwitterCoordinate *tempCoordinate;
	
	NSArray *tweetArray = [TweetFetchManager fetchFriends];

	
	for(NSInteger i = 0; i < [tweetArray count]; ++ i) {
		NSDictionary *tweet = [tweetArray objectAtIndex:i];
		
		NSLog(@"%@",tweet);
		tempLocation = [[CLLocation alloc] initWithLatitude:[[tweet valueForKey:@"lat"] floatValue] longitude:[[tweet valueForKey:@"lng"] floatValue]];
	
		tempCoordinate = [ARGeoTwitterCoordinate coordinateWithLocation:tempLocation withImageURL:[tweet valueForKey:@"img"]];
		//tempCoordinate.inclination = M_PI/10;
		tempCoordinate.title = [tweet valueForKey:@"title"];
	
		tempCoordinate.inclination = M_PI/(25 + (i));
	...
	CLLocation *newCenter = [[CLLocation alloc] initWithLatitude:34.6937378 longitude:135.5021651];
...
- (UIView *)viewForCoordinate:(ARCoordinate *)coordinate {
	...
	pointView.image = [UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:[(ARGeoTwitterCoordinate*)coordinate imageURL]]]];
